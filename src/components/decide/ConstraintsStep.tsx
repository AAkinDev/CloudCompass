...
import { Constraints } from '../../lib/recommend/score';
...
interface ConstraintsStepProps {
  ...
  onUpdate: (updates: Partial<{
    constraints: Constraints;
    budget?: number;
    scale: { monthlyRequests: number; storedGB: number };
  }>) => void;
}
...
const ConstraintsStep: React.FC<ConstraintsStepProps> = ({ constraints, budget, scale, onUpdate }) => {
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {databaseOptions.map((option) => (
            <button
              key={option.id}
              onClick={() => handleConstraintUpdate({ db: option.id as any })}
              className={`p-3 text-left rounded-lg border transition-colors ${
